...
typedef void (CUPTIAPI *CUpti_CallbackFunc)(void*, CUpti_CallbackDomain, CUpti_CallbackId, const CUpti_CallbackData*);

extern "C" {
CUptiResult CUPTIAPI cuptiSubscribe(CUpti_SubscriberHandle* subscriber, CUpti_CallbackFunc cbfunc, void* userdata);
CUptiResult CUPTIAPI cuptiUnsubscribe(CUpti_SubscriberHandle subscriber);
CUptiResult CUPTIAPI cuptiEnableDomain(unsigned int enable, CUpti_SubscriberHandle subscriber, CUpti_CallbackDomain domain);
...
//   NVIDIA docs state functionName is a global constant and valid to read. [docs]
...
#define CUPTI_CALL_THROW(call)                                                   \
  do {                                                                           \
    ...
    if (_status != CUPTI_SUCCESS) {                                              \
      ...
      if (_errstr == nullptr) _errstr = "Unknown CUPTI error";                   \
...
// Try to join a Python list[str] into a single UTF-8 std::string without allocating too much in C++.
static std::string py_list_join_to_string(PyObject* list_obj) {
  std::string out;
  if (!list_obj) return out;
  if (!PyList_Check(list_obj)) return out;
...
static void ensure_traceback_objects_held_GIL() {
  if (g_traceback_mod && g_format_stack_fn) return;
...
// We prefer PyThreadState_GetFrame (3.9+) and fall back to PyEval_GetFrame (older).
static PyObject* get_current_frame_held_GIL() {
#if PY_VERSION_HEX >= 0x03090000
  PyThreadState* tstate = PyThreadState_Get();
  if (!tstate) return nullptr;
  PyObject* frame = (PyObject*)PyThreadState_GetFrame(tstate); // new ref or NULL
  return frame;
#else
...
static bool should_log_for(const char* functionName) {
  ...
  if (g_filters.empty()) return true; // "watch all" if empty
  return (g_filters.find(functionName) != g_filters.end());
}

static const char* domain_str(CUpti_CallbackDomain d) {
  switch (d) {
    case CUPTI_CB_DOMAIN_RUNTIME_API: return "runtime";
    case CUPTI_CB_DOMAIN_DRIVER_API:  return "driver";
    default:                          return "other";
  }
}

extern "C" void CUPTIAPI cupti_callback(
    void* /*userdata*/,
  ...
  if (!cbInfo) return;
  if (tls_in_callback) return;               // prevent re-entrancy
  if (cbInfo->callbackSite != g_site.load()) return;
  if (!should_log_for(cbInfo->functionName)) return;
  ...
  if (!frame) {
    std::fprintf(stderr,
        ...
        domain_str(domain),
        g_site.load() == CUPTI_API_ENTER ? "enter" : "exit",
...
static void subscribe_if_needed(bool runtime, bool driver) {
  if (g_subscribed.load()) {
    ...
    CUPTI_CALL_THROW(cuptiEnableDomain(runtime ? 1 : 0, g_subscriber, CUPTI_CB_DOMAIN_RUNTIME_API));
    CUPTI_CALL_THROW(cuptiEnableDomain(driver  ? 1 : 0, g_subscriber, CUPTI_CB_DOMAIN_DRIVER_API));
    return;
  }
  // Ensure driver is initialized before subscribing (helps avoid CUPTI_ERROR_NOT_INITIALIZED).
  try_cuInit();
  CUPTI_CALL_THROW(cuptiSubscribe(&g_subscriber, (CUpti_CallbackFunc)cupti_callback, nullptr));
...
static bool convert_iterable_of_str(PyObject* obj, std::vector<std::string>& out, const char* what) {
  out.clear();
  if (!obj) return true; // treat NULL as empty
  ...
  Py_DECREF(it);
  if (PyErr_Occurred()) return false; // from iteration
  return true;
}

static PyObject* py_enable(PyObject* /*self*/, PyObject* args, PyObject* kwargs) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|Os", (char**)kwlist, &api_names, &domains, &site)) {
    return nullptr;
  }

  std::vector<std::string> names_vec;
  if (!convert_iterable_of_str(api_names, names_vec, "api_names")) return nullptr;

  // Update filters
  {
    std::lock_guard<std::mutex> lock(g_filters_mu);
    g_filters.clear();
    for (const auto& s : names_vec) g_filters.insert(s);
  }
  ...
  if (domains == nullptr || domains == Py_None) {
    ...
    if (!convert_iterable_of_str(domains, doms, "domains")) return nullptr;
    for (const auto& ds : doms) {
      if (ds == "runtime") runtime = true;
      else if (ds == "driver") driver = true;
      else {
        PyErr_SetString(PyExc_ValueError, "domain must be 'runtime' or 'driver'");
        return nullptr;
      }
    }
    if (!runtime && !driver) runtime = true;
  }
  ...
  // Site
  if (std::strcmp(site, "enter") == 0) g_site.store(CUPTI_API_ENTER);
  else if (std::strcmp(site, "exit") == 0) g_site.store(CUPTI_API_EXIT);
  else {
...
static PyObject* py_set_functions(PyObject* /*self*/, PyObject* args) {
  PyObject* api_names = nullptr;
  if (!PyArg_ParseTuple(args, "O", &api_names)) return nullptr;
  std::vector<std::string> names_vec;
  if (!convert_iterable_of_str(api_names, names_vec, "api_names")) return nullptr;
  std::lock_guard<std::mutex> lock(g_filters_mu);
  g_filters.clear();
...
static PyObject* py_is_enabled(PyObject* /*self*/, PyObject* /*args*/) {
  if (g_enabled.load()) Py_RETURN_TRUE;
  Py_RETURN_FALSE;
}

static PyMethodDef module_methods[] = {
    {"enable", (PyCFunction)py_enable, METH_VARARGS | METH_KEYWORDS, "Enable stack printing for selected CUDA API names."},
    {"disable", (PyCFunction)py_disable, METH_NOARGS, "Disable stack printing and detach CUPTI."},
    {"set_functions", (PyCFunction)py_set_functions, METH_VARARGS, "Update the allow-list of CUDA API names."},
    {"is_enabled", (PyCFunction)py_is_enabled, METH_NOARGS, "Return whether logging is currently enabled."},
